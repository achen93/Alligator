...
command_indented_print (int indent, command_t c)
{
  switch (c->type)
    {
    case IF_COMMAND:    // if A then B else C fi
    case UNTIL_COMMAND: // A;B (a newline can also substitute a semicolon)
    case WHILE_COMMAND: // while A do B done
      printf ("%*s%s\n", indent, "",
    ...
    case SEQUENCE_COMMAND:  // A;B (a newline can also substitute a semicolon)
    ...
    case SIMPLE_COMMAND:    // ex: cat a ... cat < a
      {
	char **w = c->u.word;
	printf ("%*s%s", indent, "", *w);
	while (*++w)
